...
class CardLabel(tk.Label):
    def __init__(self, master, card, **kw):
        ...
        text = f"{rank}\nof\n{suit}"
        super().__init__(master, text=text, bg="white", fg=fg,
                         width=8, height=4, borderwidth=2, relief="raised", **kw)
...
class GameUI(tk.Tk):
    ...
    def _build_ui(self):
        self.card_frame = tk.Frame(self); self.card_frame.pack(pady=20)
        self.info_label = tk.Label(self, text="");   self.info_label.pack()
    ...
    def _on_click(self, event):
        w = event.widget
        if not hasattr(w, "card_index"): return
        idx = w.card_index
        # play
        self.engine.play_turn(self.engine.current_player, idx)

        if self.engine.check_deal_end():
            # who actually played the last card?
            scoring = 1 - self.engine.current_player
            ...
            messagebox.showinfo("Deal Over",
                f"Player {scoring+1} scores {pts} point(s)")

            if self.engine.award_counters(scoring, pts):
                messagebox.showinfo("Match Over",
                    f"Player {scoring+1} wins the match!")
                over = tk.Toplevel(self); over.title("Game Over")
                tk.Label(over, text="🎮 Game Over 🎉",
                         font=("Helvetica",24), fg="blue",
                         padx=20, pady=20).pack()
                return
            else:
                messagebox.showinfo("Next Deal",
